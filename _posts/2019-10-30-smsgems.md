...
I first needed to find a good source for clip art images. I came across [this Microsoft clip art collection](https://archive.org/details/MS_Clipart_Collection) uploaded to archive.org. The archive contains roughly 60,000 clip art images in Windows Metafile format (WMF). Luckily, Inkscape is able to open WMF files and works in headless mode, so converting the images to SVG was trivial.
...
In order to make the covers look less generated and more like a scan I used imagemagick to overlay a paper grain texture, apply a subtle gaussian blur and rotated the picture slightly.
...
Overall I'm quite pleased with the results given the relatively small amount of work it took. The results are rarely funny, but they are often quite convincing.
